    ...
    for (uint64_t b = 0; b < batch_size; ++b) {
        for (uint64_t i = 0; i < seq_len_q; ++i) {
            ...
            for (uint64_t j_start = 0; j_start < seq_len_k; j_start += Bc) {
                ...
                __m256 v_rescale_old = _mm256_set1_ps(rescale_old);
                __m256 v_inv_l_new = _mm256_set1_ps(inv_l_new);

                uint64_t k = 0;
                for (; k + 7 < d; k += 8) {
                    __m256 pv_vec = _mm256_setzero_ps();
                    for (uint64_t j = 0; j < block_size; ++j) {
                        __m256 s_val = _mm256_set1_ps(S_block[j]);
                        __m256 v_vec = _mm256_loadu_ps(&V.at(b, j_start + j, k));
